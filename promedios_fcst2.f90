...
!   
!
!   Created by Agustin Garcia on 07/08/14.
!   Copyright 2015 Centro de Ciencias de la Atmosfera, UNAM. All rights reserved.
!
!   Lee salidas del pronostico de 2014
!   10-04-2015 Inclusion de PBL, Ozone
!
!   ifort -O3  promedios_fcst2.f90 -o prom2_fct.exe
program promedios
implicit none
...
do ih=1,n_times
  do i=1,ndays(im)
    fname=mes(im)//'00'//"-"//hora(ih)//"_pblh.txt"
...
333 continue
  ...
  close(10)
  open (20,file=mes(im)//'_'//hora(ih)//'_SO2h.csv')
  open (21,file=mes(im)//'_'//hora(ih)//'_COh.csv')
  open (22,file=mes(im)//'_'//hora(ih)//'_NO2h.csv')
  open (23,file=mes(im)//'_'//hora(ih)//'_O3h.csv')
